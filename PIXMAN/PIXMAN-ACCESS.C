                                 ...
                                 const uint32_t *mask)
{
	...
	r = (uint32_t)(to_linear[r] * 255.0f + 0.5f);
	g = (uint32_t)(to_linear[g] * 255.0f + 0.5f);
	b = (uint32_t)(to_linear[b] * 255.0f + 0.5f);
                                 ...
                                 const uint32_t *v)
{
	...
	a = (uint32_t)((tmp >> 24) & 0xff);
	r = (uint32_t)((tmp >> 16) & 0xff);
	g = (uint32_t)((tmp >> 8) & 0xff);
	b = (uint32_t)((tmp >> 0) & 0xff);
