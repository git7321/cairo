				     ...
				     const void *p, pixman_fixed_t f)
{
    ...
    *satot += (size_t)(pixel->a * f);
    *srtot += (size_t)(pixel->r * f);
    *sgtot += (size_t)(pixel->g * f);
    *sbtot += (size_t)(pixel->b * f);
}
			    ...
			    const uint32_t *mask)
{
	    ...
	    (wide && memcmp(&mask[4 * i], wide_zero, 16) != 0))
	{
	    if (w != 0)
	    {
		x0 = (pixman_fixed_t)(((uint64_t)x << 16) / w);
		y0 = (pixman_fixed_t)(((uint64_t)y << 16) / w);
	    }
	    else
	    {
		x0 = 0;
