...
fast_fetch_bilinear_cover (pixman_iter_t *iter, const uint32_t *mask)
{
	...
	uint64_t tar = (top & 0xffff0000ffff0000UI64) >> 16;
	uint64_t bar = (bot & 0xffff0000ffff0000UI64) >> 16;
	uint64_t tgb = (top & 0x0000ffff0000ffffUI64);
	uint64_t bgb = (bot & 0x0000ffff0000ffffUI64);
	uint64_t ar, gb;
	uint32_t a, r, g, b;
					       ...
					       pixman_repeat_t	repeat_mode)
{
	...
	for (i = y1; i < y2; ++i)
	{
		...
		for (j = x1; j < x2; ++j)
		{
			...
			f = (pixman_fixed_t)((fx * fy + 0x8000) >> 16);
