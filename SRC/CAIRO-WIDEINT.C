...
_cairo_uint64x64_128_mul (cairo_uint64_t a, cairo_uint64_t b)
{
    ...
    al = (uint32_t)uint64_lo32 (a);
    ah = (uint32_t)uint64_hi32 (a);
    bl = (uint32_t)uint64_lo32 (b);
    bh = (uint32_t)uint64_hi32 (b);
				 ...
				 cairo_uint64_t den)
{
    ...
    result.quo = _cairo_uint32s_to_uint64 (-1, -1);
    result.rem = den;
	...
	uint32_t u = (uint32_t)uint64_hi32(den);
...
_cairo_int_96by64_32x64_divrem (cairo_int128_t num, cairo_int64_t den)
{
	...
	qr.quo = _cairo_uint32s_to_uint64 (0x7FFFFFFF, -1);
	qr.rem = den;
	return qr;
    }

    if (num_neg)
	qr.rem = _cairo_uint64_negate (uqr.rem);
    else
	qr.rem = uqr.rem;
    if (num_neg != den_neg)
	qr.quo = _cairo_uint64_negate (uqr.quo);
    else
	qr.quo = uqr.quo;
    return qr;
}
