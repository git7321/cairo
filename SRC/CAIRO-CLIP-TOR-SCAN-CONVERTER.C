...
#define GRID_XY (2*GRID_X*GRID_Y)
...
#define INPUT_TO_GRID_general(in, out, grid_scale) do { \
	__int64 tmp__ = (__int64)(grid_scale) * (in); \
	tmp__ >>= GLITTER_INPUT_BITS; \
	(out) = (cairo_fixed_t)tmp__; \
} while (0)
		...
		int y, int height)
{
	...
	if (x > prev_x && cover != last_cover) {
	    ...
	    spans[num_spans].coverage = (uint8_t)GRID_AREA_TO_ALPHA (cover);
	...
	if (area != last_cover) {
	    spans[num_spans].x = x;
	    spans[num_spans].coverage = (uint8_t)GRID_AREA_TO_ALPHA (area);
	    spans[num_spans].inverse = 0;
	    last_cover = area;
	    ++num_spans;
	}
