				      ...
				      unsigned long          tag)
{
    ...
    cairo_truetype_font_write_be16 (font, 3);
    cairo_truetype_font_write_be16 (font, 1);
    cairo_truetype_font_write_be32 (font, 12);

    cairo_truetype_font_write_be16 (font, 4);
    cairo_truetype_font_write_be16 (font, length);
    cairo_truetype_font_write_be16 (font, 0);
    cairo_truetype_font_write_be16 (font, num_ranges*2 + 2);
    cairo_truetype_font_write_be16 (font, (1 << (entry_selector + 1)));
    cairo_truetype_font_write_be16 (font, entry_selector);
    cairo_truetype_font_write_be16 (font, num_ranges*2 + 2 - (1 << (entry_selector + 1)));
    for (i = 0; i < num_ranges; i++)
	cairo_truetype_font_write_be16 (font, winansi_unicode_ranges[i].end);
    cairo_truetype_font_write_be16 (font, 0xffff);

    cairo_truetype_font_write_be16 (font, 0);

    for (i = 0; i < num_ranges; i++)
	cairo_truetype_font_write_be16 (font, winansi_unicode_ranges[i].start);
    cairo_truetype_font_write_be16 (font, 0xffff);

    for (i = 0; i < num_ranges; i++)
	cairo_truetype_font_write_be16 (font, 0x0000);
    cairo_truetype_font_write_be16 (font, 1);

    range_offset = num_ranges*2 + 2;
    for (i = 0; i < num_ranges; i++) {
	cairo_truetype_font_write_be16 (font, range_offset);
				      ...
				      cairo_bool_t                is_pdf)
{
    ...
    truetype_subset->widths = calloc (font->scaled_font_subset->num_glyphs,
                                      sizeof (double));
...
find_name (tt_name_t *name, int name_id, int platform, int encoding, int language, char **str_out)
{
	...
	if (be16_to_cpu (record->name) == name_id &&
	    ...
	    if (platform == 3 && len > MAX_FONT_NAME_LENGTH*2)
		break;
				...
				char 	       		**font_name_out)
{
    ...
    status = find_name (name, 6, 3, 1, 0x409, &ps_name);
    if (unlikely(status))
	goto fail;

    if (!ps_name) {
	status = find_name (name, 6, 1, 0, 0, &ps_name);
	if (unlikely(status))
	    goto fail;
    }

    status = find_name (name, 1, 3, 1, 0x409, &family_name);
    if (unlikely(status))
	goto fail;

    if (!family_name) {
	status = find_name (name, 1, 3, 0, 0x409, &family_name);
	if (unlikely(status))
	    goto fail;
    }

    if (!family_name) {
	status = find_name (name, 1, 1, 0, 0, &family_name);
	if (unlikely(status))
	    goto fail;
    }

    if (!family_name) {
	status = find_name (name, 1, 3, 1, -1, &family_name);
	if (unlikely(status))
	    goto fail;
    }
