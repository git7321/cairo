					 ...
					 const cairo_rectangle_int_t *extents)
{
    ...
    surface->extents.x *= (int)target->device_transform.xx;
    surface->extents.y *= (int)target->device_transform.yy;
    surface->extents.width *= (int)target->device_transform.xx;
    surface->extents.height *= (int)target->device_transform.yy;
